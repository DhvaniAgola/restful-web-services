...
//				{ 3, 5, 20 }, { 4, 5, 4 } };
		int edges[][] = { { 0, 1, 5 }, { 0, 2, 5 }, { 1, 4, 6 }, { 1, 3, 3 }, { 2, 4, 3 }, { 2, 3, 1 }, { 3, 5, 6 },
				{ 4, 5, 6 } };
		graph g = new graph(n);
		addEdge(g, edges);
	...
	static int arr[][];
	static List<Integer> ConnectivityLst = new ArrayList<>();
...
//			System.out.println("----------------------------");
		...
		for (int i = 0; i < n; i++) {
			NodeList1.add(i);
			ConnectivityLst.add(i);
		}
		checkConnectivity(g, edges, visited2, 0, lst);
		for (int i = 0; i < visited2.length; i++)
			visited2[i] = false;
		NodeList1.removeAll(ConnectivityLst);
		System.out.println("Nodes connected with 's' : " + NodeList1);
		System.out.println("Nodes connected with 't' : " + ConnectivityLst);

		return max;
	}

	private static void checkConnectivity(graph g, int[][] edges, boolean[] visited2, int s, List<Integer> lst) {
		// TODO Auto-generated method stub
		visited2[s] = true;
		boolean vTemp[] = new boolean[visited2.length];
		...
		lst.add(s);
		ConnectivityLst.remove(new Integer(s));
		lstTemp.addAll(lst);
		...
		for (int i : g.ll[s]) {
			...
			if (!visited2[i]) {
				for (int j = 0; j < edges.length; j++) {
					if (edges[j][0] == s && edges[j][1] == i && edges[j][2] > 0) {
						f = true;
					}
				}
				if (f) {
					checkConnectivity(g, edges, vTemp, i, lstTemp);
